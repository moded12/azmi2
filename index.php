...
// داخل الحلقة while … else (أي عند جمع الملفات)
$link = ownurlencode($dir."/".$list);
$file_html = '';

// إذا كانت وسائط مفعّلة، يبقى الكود القديم لعرض الصورة/صوت/فيديو
if ($media == "1") {
  // … الكود الحالي  
}

// بعد ذلك نضيف:
$ext = strtolower(pathinfo($list, PATHINFO_EXTENSION));
$thumb = '';
...
// ثم ندفع العنصر للمصفوفة مع المصغّرة
array_push($files,
    '<a class="files" href="'.$link.'">'.nb($list).'</a>'
  . ' ~ '.format_size(filesize($link))
  . $file_html
  . $thumb    // هذه هي الإضافة
);
unset($file_html);
